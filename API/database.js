...
export async function gameById(id) {
    const [rows] = await pool.query(`
                SELECT JOGO.ID_JOGO, JOGO.NOME, DESCRIÇÃO, DATA_LANÇAMENTO, MEMBROS, ID_FOTO, ID_ESTÚDIO, avg(AVALIAÇÃO.NOTA) AS NOTA, GÊNERO.NOME AS NOME_GÊNERO
                FROM PCS_BD.JOGO LEFT JOIN PCS_BD.AVALIAÇÃO ON AVALIAÇÃO.ID_JOGO = JOGO.ID_JOGO LEFT JOIN PCS_BD.GÊNERO_JOGO ON GÊNERO_JOGO.ID_JOGO = JOGO.ID_JOGO LEFT JOIN GÊNERO ON GÊNERO.ID_GÊNERO = GÊNERO_JOGO.ID_GÊNERO
                WHERE JOGO.ID_JOGO = ?
