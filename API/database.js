...
export async function gameById(id) {
    const [rows] = await pool.query(`
                SELECT JOGO.ID_JOGO, JOGO.NOME, DESCRIÇÃO, DATA_LANÇAMENTO, MEMBROS, JOGO.ID_FOTO, JOGO.ID_ESTÚDIO, ESTÚDIO.NOME AS NOME_ESTUDIO, ESTÚDIO.ID_FOTO AS FOTO_ESTUDIO, avg(AVALIAÇÃO.NOTA) AS NOTA, GÊNERO.NOME AS NOME_GÊNERO
                FROM PCS_BD.JOGO LEFT JOIN PCS_BD.AVALIAÇÃO ON AVALIAÇÃO.ID_JOGO = JOGO.ID_JOGO LEFT JOIN PCS_BD.GÊNERO_JOGO ON GÊNERO_JOGO.ID_JOGO = JOGO.ID_JOGO LEFT JOIN GÊNERO ON GÊNERO.ID_GÊNERO = GÊNERO_JOGO.ID_GÊNERO LEFT JOIN ESTÚDIO ON ESTÚDIO.ID_ESTÚDIO = JOGO.ID_ESTÚDIO
                WHERE JOGO.ID_JOGO = ?
...
export async function getUserByUsername(username) {
    ...
    const [rows] = await pool.query(
        "SELECT * FROM PCS_BD.USUÁRIO WHERE NOME = ?", 
        [username])
    return rows[0]
}
